    ...
    const countNonZeroAnnotations = () => {
      ...
      DataCollected.data_collected.forEach(category => {
        ...
        category[categoryName].forEach(item => {
          ...
          if (matchingData && String(matchingData.data_collection_category_assessed).toLowerCase() === "yes") {
            nonZeroCount++;
          }
        });
      });
      ...
      return (
        <Grid container key={index}>
          ...
          <Grid item xs={12} sm={2} md={2}>
            <Typography className={classes.number}>{matchingData.data_collection_category_assessed}</Typography>
          </Grid>
        </Grid>
      );
    });
